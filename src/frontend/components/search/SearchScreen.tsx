...
export function SearchScreen() {
  ...
  return (
    <div className="grid gap-4">
      ...
      {deferredQuery && results.isError && <BackendUnavailable resource="Search results" />}
      {deferredQuery && results.isLoading && <p role="status" className="text-sm opacity-60">Searching…</p>}
      {deferredQuery && results.data?.length === 0 && <p role="status" className="text-sm opacity-60">No matching records were returned.</p>}
      {results.data && results.data.length > 0 && <ul className="grid gap-2">{results.data.map((meeting) => <li key={meeting.id} className="card border border-base-300 bg-base-200"><div className="card-body flex-row items-center justify-between p-3"><div><strong>{meeting.title}</strong><div className={`text-sm ${statusText[meeting.status]}`}>{statusLabel[meeting.status]}</div></div><Link className="btn btn-outline btn-sm" href={`/app/meetings/${meeting.id}`}>Open</Link></div></li>)}</ul>}
    </div>
  );
}
