...
export function ReviewHistoryTimeline({ history }: { history: HistoryEntry[] }) {
  if (!history.length) {
    return <p className="py-8 text-center text-sm text-muted-foreground">No review activity has been recorded for this meeting.</p>;
  }
      ...
      {newestFirst(history).map((entry, index, all) => (
        <li key={entry.id} className="relative flex gap-3 pb-4">
          ...
          <div className="min-w-0 text-sm">
            <p><strong>{actionLabel[entry.action]}</strong><span className="text-muted-foreground"> — {entry.actorDisplayName}</span></p>
            <time dateTime={entry.createdAt} className="text-xs text-muted-foreground">{formatAt(entry.createdAt)}</time>
